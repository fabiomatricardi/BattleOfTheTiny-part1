...
with gr.Blocks(theme='Medguy/base2') as demo:   #theme=gr.themes.Glass()  #theme='remilia/Ghostly'
    ...
    with gr.Row(variant='compact'):
            with gr.Column(scale=3):            
                gr.Image(value='./lamini248.jpg', 
                        show_label = False, 
                        show_download_button = False, container = False)              
            with gr.Column(scale=10):
                gr.HTML("<center>"
                + "<h3>Prompt Engineering Playground!</h3>"
                + "<h1>🦙 LaMini-Flan-T5-248M - 512 context window</h1></center>")  
                with gr.Row():
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            max_len = gr.Slider(label="Maximum output lenght", minimum=10,maximum=(contextlength-50),step=2, value=900)
            gr.Markdown(
            """
            ...
            - **Log File**: [{logfile}](file/LaMini248M_logs.txt)
            """) 
