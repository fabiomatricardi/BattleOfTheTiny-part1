...
convHistory = ''
modelfile = "models/tinyllama-2-1b-miniguanaco.Q5_K_M.gguf"
repetitionpenalty = 1.15
contextlength=2048
logfile = 'tinyllama-2-1b-miniguanaco_logs.txt'
print("loading model...")
...
with gr.Blocks(theme='Medguy/base2') as demo:   #theme=gr.themes.Glass()  #theme='remilia/Ghostly'
    ...
    with gr.Row(variant='compact'):
            with gr.Column(scale=3):            
                gr.Image(value='https://github.com/fabiomatricardi/TiniLlamaGradioChat/raw/main/TinyGuanaco.jpg', 
                        show_label = False, height = 150,
                        show_download_button = False, container = False,)              
            with gr.Column(scale=10):
                gr.HTML("<center>"
                + "<h3>Prompt Engineering Playground!</h3>"
                + "<h1>🦙 Tinyllama-2-1b-Miniguanaco - 2048 context window</h2></center>")  
                with gr.Row():
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            gr.Markdown(
            f"""
            - **Prompt Template**: Llama instruct 🦙
            - **Repetition Penalty**: {repetitionpenalty}
            - **Context Lenght**: {contextlength} tokens
            - **LLM Engine**: llama-cpp
            - **Model**: 🦙 tinyllama-2-1b-miniguanaco.Q5_K_M.gguf
            - **Log File**: {logfile}
            """) 


        with gr.Column(scale=4):
            txt = gr.Textbox(label="System Prompt", lines=1, interactive = False, value='', placeholder='This models does not have a system prompt...')
